...
* This preserves the desired capacity while existing and replacement Pods remain healthy. It cannot guarantee availability during node failures, bad images, or cluster outages.
...
## Production Use Case
* **Any stateless microservice** doing a new feature release or bug fix — controlled updates to the order service, payment API, or authentication service while healthy replacement Pods are available.
...
## Cleanup
```bash
kubectl delete -f rollingupdate/service.yaml
kubectl delete -f rollingupdate/deployment-v1.yaml
kubectl delete -f rollingupdate/deployment-v2.yaml
```